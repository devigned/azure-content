...
Provision Linux clusters from a computer running this OS| Using the Azure portal | Using the Azure Command-Line Interface (Azure CLI) | Using the .NET SDK | Using Azure PowerShell
-----------------| ------------------------| -------------------| ---------- | ---------
...
Another option for provisioning an HDInsight cluster is the Azure CLI. The command-line tool is implemented in Node.js. It can be used on any platform that supports Node.js, including Windows, Mac and Linux. You can install the Azure CLI from the following locations:
...
For a general guide on how to use the Azure CLI, see [Azure CLI](azure-cli.md).
...
Perform the following procedures to set up your Linux computer to use Azure CLI:
...
Perform the following procedures to set up your Windows computer to use the Azure CLI:

- Install the Azure CLI by using NPM or Windows Installer
- Download and import Azure account publish settings


The Azure CLI can be installed via NPM or Windows Installer. Microsoft recommends that you install it by using only one of the two options.
...
**To install the Azure CLI by using Windows Installer**
...
[azure-command-line-tools]: ../azure-cli/
