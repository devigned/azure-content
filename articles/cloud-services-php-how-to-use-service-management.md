...
If you have downloaded and imported your publish settings file using the [Azure Command-Line Interface (Azure CLI)][command-line-tools], you can use the `.pem` file that the tools create instead of creating your own. The tools create a `.cer` for you and upload it to Azure, and they put the corresponding `.pem` file in the `.azure` directory on your computer (in your user directory).
